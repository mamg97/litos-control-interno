...
def assert_write_safety() -> None:
    ...
    if not _flag("LITOS_SABAN_WRITE_ENABLED"):
        raise RuntimeError("LITOS_SABAN_WRITE_ENABLED must be true")
    if _flag("LITOS_SABAN_KILL_SWITCH"):
        raise RuntimeError("LITOS_SABAN_KILL_SWITCH is active")
...
def sync_intake(*, max_mutations: int | None = None) -> dict:
    assert_write_safety()
    allowed_sender = os.environ.get("SABAN_ALLOWED_SENDER", "").strip().lower()
    if not re.fullmatch(r"[^\s@]+@[^\s@]+\.[^\s@]+", allowed_sender):
        raise RuntimeError("SABAN_ALLOWED_SENDER is missing or invalid")
    ...
    for message in messages:
        ...
        try:
            ...
            set_if_blank(services.sheets, orders, row_no, HEADERS["receipt_origin"], "Gmail · saban", counters)
            set_if_blank(
                services.sheets,
                orders,
                row_no,
