...
def assert_write_safety() -> None:
    ...
    if _flag("LITOS_CLIENT_KILL_SWITCH"):
        raise RuntimeError("LITOS_CLIENT_KILL_SWITCH is active")
...
def ensure_review_row(sheets, review_state: dict, order_id: str, receipt_date: str, note_file_id: str, counters: dict) -> None:
    ...
    values = {
        ...
        "Fuente de lectura": "Gmail · client",
...
def sync_intake(*, max_mutations: int | None = None) -> dict:
    assert_write_safety()
    allowed_sender = os.environ.get("CLIENT_ALLOWED_SENDER", "").strip().lower()
    ...
    for message in messages:
        ...
        try:
            ...
            set_if_blank(services.sheets, orders, row_no, HEADERS["receipt_origin"], "Gmail · client", counters)
            set_if_blank(
                services.sheets,
                orders,
                row_no,
