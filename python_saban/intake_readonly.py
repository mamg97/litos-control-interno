...
ROOT_FOLDER_ID = os.environ.get("LITOS_SABAN_FOLDER_ID", "1eUAupqLzfBhkiEexWqpI3JtYReT8c9A_")
LABEL = os.environ.get("LITOS_SABAN_LABEL", "saban")
YEAR = int(os.environ.get("LITOS_SABAN_YEAR", "2026"))
LOOKBACK_DAYS = int(os.environ.get("LITOS_SABAN_LOOKBACK_DAYS", "30"))
...
def list_candidate_messages(gmail, allowed_sender: str) -> list[dict]:
    query = f"label:{LABEL} from:{allowed_sender} newer_than:{LOOKBACK_DAYS}d"
    threads: list[dict] = []
    page_token = None
    while True:
        response = (
...
def build_read_only_plan(services, allowed_sender: str) -> dict:
    ...
    return {
        "mode": "SABAN_READ_ONLY_DISCOVERY",
        "year": YEAR,
