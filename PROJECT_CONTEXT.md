...
- Full published-data audit: 925/925 orders, 244/244 expenses and 1080 account movements are present after the latest running-account reconciliation; there are no missing IDs, extra IDs or duplicate canonical four-digit IDs. The repeated noncanonical marker `—` represents intentional ledger-only/payment entries. Currency is published at displayed two-decimal precision. A Google XLSX export can reinterpret a few slash-formatted values as dates; direct Google Sheets reads confirmed the public displayed values, including 7740 step measures and the affected account balances.
- Link audit covered 1,521 unique URLs (all feed document/expense links plus seven year-folder links): 1,359 returned HTTP 200 and 162 private Google links returned HTTP 401; there were no 404/410/5xx responses, invalid hosts or missing local assets. The single repeated document URL is intentional for the combined 7003/7005 source. All external links use `noopener noreferrer`.
- Web audit: local static build and JavaScript syntax check pass; the deployed page loads 925 records without console errors, all five views switch correctly, 2020 shows 9/13/23/6/11 = 62, search returns 7041 exactly once, and its row shows no delivery date, order date 15/10/2020, PVP 0 and the historical-close label. The finance view shows the latest reconciled movement first and its displayed balance matches the authoritative estadillo. Privacy/OAuth routes and static assets are included in the build.
...
- Supplier-expense checkpoint completed on 2026-09-21. The 2026 business mailbox was reviewed for incoming invoice/credit-note attachments and the directly supplied current invoice was inspected separately. Eleven authoritative mailbox documents plus the directly supplied document were archived under `Facturas proveedores/2026/<proveedor>` and added to `Gastos`; superseded invoice versions, a proforma and non-invoice attachments were excluded from expense totals. The verified master now contains 256 expense rows.
- None of the newly registered supplier documents contains a trustworthy LITOS work ID, and description searches did not establish a unique order match. They therefore remain stock, general-service or pending-link supplier expenses and do not overwrite any `Coste directo real (€)` value. A supplier credit note is recorded as a negative expense against the corrected stock purchase. The private master retains the email/document trace; the public feed continues to omit message URLs and private document contents.
- Mailbox review checkpoint: invoice-focused incoming-mail coverage is current through 2026-09-21. A newly received catalogue/tariff is operational reference material, not a realized expense. Future reviews should start after this checkpoint while still checking thread corrections and duplicate references before adding rows.
