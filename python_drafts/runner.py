...
EXPECTED_PRIVATE_CONFIG_SHEET = "Configuracion privada"

def _private_customer_profile(sheets) -> dict:
    values = (
        sheets.spreadsheets()
        .values()
        .get(
            spreadsheetId=EXPECTED_SPREADSHEET_ID,
            range=f"'{EXPECTED_PRIVATE_CONFIG_SHEET}'!A:B",
    ...
    for row in values[1:]:
        if len(row) >= 2:
            ...
            if key:
                config[key] = value
    profile = {
        "name": config.get("cliente_nombre", ""),
        "address": config.get("cliente_direccion", ""),
        "city": config.get("cliente_ciudad", ""),
        "province": config.get("cliente_provincia", ""),
    }
    if any(not value for value in profile.values()):
        raise RuntimeError("Private client profile is incomplete in master configuration")
    return profile
...
def _planner_api():
    ...
    if _truthy(os.getenv("LITOS_DRAFT_CORPORATE_STYLE", "false")):
        ...
        def _styled_target(target: dict, customer: dict) -> dict:
            ...
            styled = apply_client_header(styled, customer)
            out = dict(target)
        ...
        def corporate_build_plan(services):
            customer = _private_customer_profile(services.sheets)
            ...
            for item in plan:
                ...
                styled_target = _styled_target(target, customer)
