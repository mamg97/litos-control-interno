...
CUSTOMER_PROFILE_VERSION = "saban-zafra-v1"
DEFAULT_CUSTOMER = {
    "name": "D MATEO PEINADO-SABAN ALVAREZ",
    "address": "C/AVDA DE LA ESTACION Nº23",
    "city": "ZAFRA",
    "province": "BADAJOZ",
}
...
def apply_client_header(xlsx_bytes: bytes, customer: dict | None = None) -> bytes:
    ...
    """
    profile = dict(DEFAULT_CUSTOMER)
    if customer:
        profile.update({k: _text(v) for k, v in customer.items() if k in profile})
    if any(not _text(profile[key]) for key in ("name", "address", "city", "province")):
...
def _build_self_test_workbook() -> bytes:
    ...
    ws["A1"] = "Miguel Angel Mayordomo Franco"
    ws.merge_cells("A2:G2")
    ws["A2"] = "N.I.F. 33973954X"
    ws.merge_cells("A3:E3")
    ws["A3"] = "Calle San José nº 12"
    ws.merge_cells("F3:G3")
    ws["F3"] = "Tlf y fax. 924 52 78 51"
    ws.merge_cells("A4:G4")
    ws["A4"] = "VILLAFRANCA DE LOS BARROS (Badajoz)"
...
def self_test() -> dict:
    result = apply_client_header(_build_self_test_workbook())
    wb = load_workbook(BytesIO(result), data_only=False)
    ws = wb.active
    assert ws["E1"].value == "CLIENTE"
    assert ws["E2"].value == DEFAULT_CUSTOMER["name"]
    assert ws["E3"].value == DEFAULT_CUSTOMER["address"]
    assert ws["E4"].value == DEFAULT_CUSTOMER["city"]
    assert ws["E5"].value == DEFAULT_CUSTOMER["province"]
