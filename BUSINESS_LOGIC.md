...
`PEDIDOS LITOS` is the private source of truth for orders.
...
The production intake processes only the explicitly authorized sender configured in repository secrets.
...
- authentication credentials belong only in GitHub Secrets.
